...
void keypadfunction(){
  ...
  */
  if (start_st == HIGH){lcd.clear(); minutes = mymints.toInt();Serial.println(minutes);
                      lcd.clear();lcd.print("Minutes: ");lcd.setCursor(0,1);lcd.print(minutes);mymints = ""; delay(2000);
                      lcd.clear();lcd.setCursor(0,0);lcd.print("Enter Seconds:");counter = 0;secflag = 1;while(secflag == 1){forSeconds();}
                      }  
  
  if (start_b == HIGH){lcd.clear(); minutes = mymints.toInt();Serial.println(minutes);lcd.clear();lcd.print("klik ulang ");mymints = ""; counter = 0;secflag = 1;while(secflag == 1){forSeconds();}}  
  }
